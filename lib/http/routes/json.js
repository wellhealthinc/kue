...
function getSearch() {
  ...
  return search = reds.createSearch(queue.client.getKey('search'), { nlpProcess: dumbNlpProcess });
}

function dumbNlpProcess(data, key) {
  let words;
  try {
  ...
  var map = {};
  words.forEach((word) => { map[word] = word });
  var keys = words;
  var metaphoneKeys = !key ? null : words.map((word) => `${key}:word:${word}`);
  return {
    words,
    counts,
    map,
